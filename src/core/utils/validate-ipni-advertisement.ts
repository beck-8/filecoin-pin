...
export type IndexingConfirmationProgressEvents =
  ...
  | ProgressEvent<
    ...
    >
  | ProgressEvent<'pieceSyncStatus:complete', { result: true; retryCount: number }>
  | ProgressEvent<'pieceSyncStatus:failed', { error: Error }>
...
export async function waitForIndexingConfirmation(
  ...
  const totalAttempts = serviceUrls.length * maxAttempts
  let totalChecks = 0
    ...
    await Promise.all(
      serviceUrls.map((serviceURL, index) =>
        waitForPieceSynced(serviceURL, pieceCid, {
          ...
          providerCount: serviceUrls.length,
          totalAttempts,
...
async function waitForPieceSynced(
  ...
  const { delayMs, maxAttempts, providerIndex, providerCount, totalAttempts, signal, onRetryUpdate, options } = config
  let retryCount = 0
  let lastFailureReason: string | undefined
  ...
  while (true) {
    ...
    const emitted = onRetryUpdate?.()
    try {
      options?.onProgress?.({
        type: 'pieceSyncStatus:retryUpdate',
        data: {
          retryCount: emitted?.retryCount ?? retryCount,
          attempt: emitted?.attempt ?? retryCount + 1,
          totalAttempts,
          serviceURL,
          providerIndex,
          providerCount,
